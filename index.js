...
const BotName = 'ESTER - BOT DIEGO MODZ'; 
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] DIEGO MODZ? Este é um recurso especial para o DIEGO MODZ ❌',
					ownerB: '[❗] DIEGO MODZ? Este é um recurso especial para o DIEGO MODZ ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de DIEGO MODZ. Registre-se para fazer amizade com o bot DIEGO MODZ por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar DIEGO|15\n\n──「 ESTER BOT 」──`,
			...
			const premium = ["5511998346168@s.whatsapp.net","+55 17 99189-6064@s.whatsapp.net"]
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/DIEGOjokes.js');
...
//@DIEGOYT					
    ...
    break
				...
				case 'textDIEGO':
					...
					if (tels.ength > 10) return reply('O texto é longo, até 9 caracteres')
					reply(mess.wait)
					anu = await fetchJson(`http://melodicxt.herokuapp.com/api/txtcustom?theme=metal_DIEGO_gold&text=${tels}&apiKey=administrator`, {method: 'get'})
					buff = await getBuffer(anu.result)
					client.sendMessage(from, buff, image, {quoted: mek})
					break
				case 'textblue':
...
// recursos anime, random @DIEGO MODZ domina irmão
          ...
          reply('Espere..')
				...
				if (!isGroup) return reply(mess.only.group)
					...
					break
			    case 'owner':
                    client.sendMessage(from, {displayname: "DIEGO MODZ", vcard: vcard}, MessageType.contact, { quoted: mek})
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o DIEGO MODZ ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMIN BOT ESTER - DIEGO MODZ 🇧🇷* 」\n*│+ wa.me/5511998346168*╰──────*「 *DIEGO MODZ* 」*────*\n\n*_SE QUER SER ADMIN DO BOT ESTER_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
